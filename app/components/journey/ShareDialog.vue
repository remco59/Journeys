...
<template>
  <dialog ref="dialog" class="w-[28rem] max-w-[90vw] rounded-2xl border border-(--color-line) bg-(--color-paper-raised) p-0 text-(--color-ink) backdrop:bg-black/30">
    <div class="flex flex-col gap-3 p-6">
      ...
      <button class="self-start rounded-lg bg-(--color-ink) px-4 py-2 text-sm text-(--color-paper)" @click="createLink">
        New share link
      </button>
      ...
      <ul v-if="shares.length" class="mt-2 flex flex-col gap-2">
        <li
          ...
          :key="share.id"
          class="flex items-center justify-between gap-2 rounded-lg border border-(--color-line) px-3 py-2 text-sm"
        ...
        >
          ...
          <span class="flex shrink-0 gap-2">
            ...
            <template v-else>
              ...
              <button class="text-red-600 hover:text-red-700" @click="revoke(share.id)">Revoke</button>
            </template>
          </span>
        </li>
      </ul>
